...
#import "NXLTenant.h"
#define UI_COM_RMS_CONFIG            @"RMS"
#define UI_TENANT_NAME           @"tenant name"
#define UI_BOX_OK                @"OK"
#define UI_COM_RESET  @"Reset"
...
- (void) commitInit {
    self.navigationItem.title = UI_COM_RMS_CONFIG;
    self.navigationItem.title = NSLocalizedString(@"UI_COM_RMS_CONFIG", nil);
    ...
    //    _rmsSiteURLTextField.placeholder = NSLocalizedString(@"UI_RMS_URL", nil);
    ...
    tenantIdLabel.text = [NSString stringWithFormat:@"%@:", NSLocalizedString(@"UI_ACCOUNT_INFO_TENANT_ID", NULL)];
    ...
    _tenantNameTextField.text = [NXLCommonUtils currentTenant];
    //    _tenantNameTextField.placeholder = NSLocalizedString(@"UI_TENANT_NAME", nil);
    ...
    [_configButton setTitle:NSLocalizedString(@"UI_BOX_OK", nil) forState:UIControlStateNormal];
    ...
    [_resetToDefaultButton setTitle:NSLocalizedString(@"UI_COM_RESET", nil) forState:UIControlStateNormal];
...
-(void) userDidClickOK:(UIButton *) button
{
    ...
    if ([self.tenantNameTextField.text isEqualToString:@""]) {
        [NXLCommonUtils showAlertViewInViewController:self title:[NXLCommonUtils currentBundleDisplayName] message:@"Please enter tenant name"];
        return;
    }
    
    [NXLCommonUtils updateTenant:self.tenantNameTextField.text];
...
{
    [_configButton resignFirstResponder];
    [NXLCommonUtils updateTenant:DEFAULT_TENANT_ID];
}
