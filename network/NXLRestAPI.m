...
NXLRestAPI* nxlgRestAPI = nil;

@implementation NXLRESTAPIVersion

@end


@interface NXLURLConnection()<NSURLSessionDelegate, NSURLSessionTaskDelegate, NSURLSessionDataDelegate>
{
...
@implementation NXLURLConnection
...
@interface NXLRestAPI ()
{
    NXLURLConnection* _conn;
}

@end

@implementation NXLRestAPI

+ (NXLRESTAPIVersion*) versionMake:(int)major minor:(int)minor maintenance:(int)maintenance patch:(int)patch build:(int)build
{
    NXLRESTAPIVersion* v = [[NXLRESTAPIVersion alloc]init];
    v.major = major;
    v.minor = minor;
...
{
    if (self = [super init]) {
        _conn = [[NXLURLConnection alloc] init];
        _conn.delegate = self;
    }
...
- (NSString *) generateRegisterAgentXml:(NSString*) rmserver host: (NSString *)host version:(NXLRESTAPIVersion *)ver
{
