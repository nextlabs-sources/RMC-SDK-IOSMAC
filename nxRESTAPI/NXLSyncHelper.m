...
-(void) cacheRESTAPI:(NXLSuperRESTAPIRequest *) restAPI cacheURL:(NSURL *) cacheURL
{
    dispatch_async(_uploadPerviousFailedRESTQueue, ^{
        [NXLCacheManager cacheRESTReq:restAPI cacheURL:cacheURL];
...
-(void) uploadPreviousFailedRESTRequestWithCachedURL:(NSURL *) cachedURL mustAllSuccess:(BOOL) mustAllSuccess Complection:(UploadFailedRESTRequestComplection) complectionBlock
{
    dispatch_async(_uploadPerviousFailedRESTQueue, ^{
        ...
        fileList = [fileList filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:[NSString stringWithFormat:@"self.absoluteString ENDSWITH '%@'", NXLSDK_CACHE_EXTENSION]]];
        ...
        dispatch_apply(fileList.count, queue, ^(size_t index) {
            ...
            if (restReqData) {
                ...
                NXLSuperRESTAPIRequest *restAPI = [NSKeyedUnarchiver unarchiveObjectWithData:restReqData];
                [restAPI requestWithObject:restAPI.reqBodyData Completion:^(NXLSuperRESTAPIResponse *response, NSError *error) {
                    if ([response isKindOfClass:[NXLSuperRESTAPIResponse class]]) {
                        NXLSuperRESTAPIResponse *restResponse = (NXLSuperRESTAPIResponse *) response;
                        if (!error && restResponse.rmsStatuCode != 0 && restResponse.rmsStatuCode != 200) {  // no net error, but have service error
                            error = [NSError errorWithDomain:NXLSDKErrorRestDomain code:NXLSDKErrorFailedUploadLog userInfo:nil];
                        }
